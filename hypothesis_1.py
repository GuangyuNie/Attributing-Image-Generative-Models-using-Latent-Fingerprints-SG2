...
class watermark_optimization:
    ...
    def generate_with_alpha(self, alpha, u_cap_t, fixed_sigma, pc, noise, difference):
        """
        v_testing:511
        ...
        new_latent = true_latent + fixed_sigma * pc[difference,:].t()
    ...
    def store_results(self, w0_reconstructed,difference,iter=None):
        if iter == None:
            pass
        else:
            self.save_dir_i = self.save_dir + 'test_{}/'.format(difference)
        ...
        img_name = self.save_dir_i + "img_{}.png".format(iter)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--key_len", type=int, default=0, help="Number of digit for the binary key"
    )
    ...
    fixed_sigma = 1
    differences = [0,1,3,7,15,31,63,127,255,511]
    for difference in differences:
        for iter in tqdm(range(10000)):
            ...
            img = optim.generate_with_alpha(rand_alpha, u_cap_t, fixed_sigma, pc, noise,difference)
            img = optim.make_image(img)
            optim.store_results(img, difference,iter)
