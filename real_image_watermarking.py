...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sd", type=int, default=2, help="Standard deviation moved"
    )
    parser.add_argument(
        "--n", type=int, default=20, help="Number of samples for Latin hypercube sampling method"
    )
    parser.add_argument(
        "--num_tests", type=int, default=10, help="Number of tests, plz make sure you have enough test samples"
    )
    ...
    args.test_dir = args.test_dir +"key_{}_sd_{}_aug_{}/".format(args.key_len,args.sd,args.augmentation)
    args.save_dir = args.save_dir +"key_{}_sd_{}_aug_{}/".format(args.key_len,args.sd,args.augmentation) # Todo: Delete this when publish
    ...
    for iter in range(tests):
        ...
        cosine_total = []
        l2_total = []
        image = Image.open('./test_images/lena_color.tiff'.format(iter)).convert('RGB').resize((256,256))
        ...
        for alpha in sample:
            ...
            theta = torch.zeros(1,dtype=torch.float32,device=optim.device)
            key.requires_grad = True
            theta.requires_grad = True
            optimizer = torch.optim.Adam([alpha, key,theta], lr=optim.lr)
