...
class watermark_optimization:
    ...
    def store_results(self, original_image_w0, original_image_wx,reconstructed_w0,reconstucted_wx, watermark_img, iter):
        ...
        store_path_watermark = 'attack_image/'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default='./robust_result_images/', help="Directory for image saving"
    )
    ...
    #     torch.manual_seed(1346)
        ...
        for iter in range(tests):
            ...
            targe_wx_img_attack = optim.make_image(optim.augmentation(targe_wx_img))
            watermark_img = 50*np.uint8(np.abs(np.int16(targe_wx_img) - np.int16(target_w0_img)))
            ...
            optim.store_results(target_w0_img,targe_wx_img,w0_reconstructed, wx_reconstructed, targe_wx_img_attack, iter)
