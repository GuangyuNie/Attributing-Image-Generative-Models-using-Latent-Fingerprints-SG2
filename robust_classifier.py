...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--augmentation", type=str, default='Crop',
    ...
    for shift in shifts:
        ...
        fixed_sigma = 4
        expectation = 10
        args.save_dir = args.save_dir + "fixed_sigma_{}/test_2/shift_{}/".format(fixed_sigma,shift).replace('.', '')
        ...
        for iter in range(tests):
            ...
            for alpha in sample:
                ...
                for i in tqdm(range(optim.steps)):
                    ...
                    loss_total_i = []
                    # expectation
                    for j in range(expectation):
                        estimated_image = optim.augmentation(estimated_image)
                        loss_1 = optim.get_loss(target_img, estimated_image, loss_func="perceptual")
                        loss_total_i.append(loss_1 + 0.1 * optim.penalty_1(alpha, max_alpha, min_alpha))
                    loss_total = min(loss_total_i)
