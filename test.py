...
class watermark_optimization:
    def __init__(self, args):
        ...
        self.fix_sigma = True
        # Get generator
        with dnnlib.util.open_url(self.ckpt) as fp:
            g_ema = legacy.load_network_pkl(fp)['G_ema'].requires_grad_(False).to(self.device)  # type: ignore
    ...
    def PCA(self):
        ...
        if os.path.isfile('./PCA/pca.pt'):
            ...
            with torch.no_grad():
                ...
                latent_out = self.g_ema.mapping(noise_sample,0)  # get style vector from Z
        ...
        return sigma_64, v_cap, u_cap, pc, sigma_512, latent_mean
    ...
    def generate_with_alpha(self, alpha, u_cap_t, sigma_64, v_cap, noise):
        ...
        latent_in = new_latent.repeat([2, 16, 1])
        imgs, _ = self.g_ema.synthesis(
            latent_in,noise_mode='const')
    ...
    def generate_image(self, style_vector, noise):
        ...
        new_latent = style_vector.repeat([2, 16, 1])
        imgs, _ = self.g_ema.synthesis(
            new_latent,noise_mode='const')
        return imgs
    ...
    def store_results(self, original_image_w0, original_image_wx,reconstructed_w0,reconstucted_wx, iter):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt", type=str, default='./checkpoint/afhqwild.pkl', required=False, help="path to the model checkpoint"
    )
    parser.add_argument(
        "--img_size", type=int, default=512, help="output image sizes of the generator"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--steps", type=int, default=20, help="Number of optimization steps"
    )
    parser.add_argument(
